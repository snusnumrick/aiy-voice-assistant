...
def log_tts_usage(text: str, engine: str, lang: str, tone: str, filename: str, api_version: str = 'v3'):
    ...
    Args:
        ...
        engine: Engine name (e.g., 'YandexTTSEngine')
        lang: Language
        tone: Tone
...
class GoogleTTSEngine(TTSEngine):
    ...
    async def synthesize_async(
        ...
        response = await loop.run_in_executor(
            None, self.client.synthesize_speech, synthesis_input, voice, audio_config
        )
...
class YandexTTSEngine(TTSEngine):
    ...
    def synthesize(
        ...
        """
        Synthesize speech using Yandex TTS (v1 or v3 based on config) and save it to a file.
...
class ElevenLabsTTSEngine(TTSEngine):
    ...
    def __init__(self, config: Config):
        ...
        if not self.api_key:
            raise ValueError(
                "ElevenLabs API key is not provided in environment variables or configuration"
            )

        # Initialize voice IDs for different languages
        self.voice_ids = {
            Language.ENGLISH: config.get(
                "elevenlabs_voice_id_en", "N2lVS1w4EtoT3dr4eOWO"
            ),
            Language.GERMAN: config.get(
                "elevenlabs_voice_id_de", "Ay1WwRHxUsu3hEeAp8JZ"
            ),
        ...
        if not all(self.voice_ids.values()):
            raise ValueError(
                "Voice IDs for all languages must be provided in the configuration"
            )
        ...
        self.query = (
            '{"output_format":config.get("elevenlabs_output_format", "mp3_22050_32")}'
        )
    ...
    def _validate_config(self) -> None:
        ...
        if (
            not isinstance(self.similarity_boost, float)
            or not 0 <= self.similarity_boost <= 1
        ):
            ...
            async with session.get(url, headers=headers) as response:
                if response.status != 200:
                    raise ElevenLabsAPIError(
                        "Invalid API key or unable to connect to ElevenLabs API"
                    )
    ...
    def _get_history_items(self, voice_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get history items: {response.status_code} - {response.text}"
            )
            raise ElevenLabsAPIError(f"Failed to get history items: {str(e)}")

    async def _find_matching_history_item_async(
        self, session: aiohttp.ClientSession, text: str, voice_id: str
    ) -> Optional[str]:
        """
    ...
    async def _download_audio_async(
        self, session: aiohttp.ClientSession, history_item_id: str, filename: str
    ) -> None:
        """
        ...
        async with session.get(url, headers=headers) as response:
            if response.status == 200:
                ...
                raise ElevenLabsAPIError(
                    f"Failed to download audio: {response.status} - {await response.text()}"
                )
    ...
    async def synthesize_async(
        ...
        try:
            history_item_id = await self._find_matching_history_item_async(
        ...
        for attempt in range(self.max_retries):
            try:
                async with session.post(
                    url, json=data, headers=headers, params=self.query
                ) as response:
                    if response.status == 200:
                        mp3_filename = f"{filename}.mp3"
                        ...
                        retry_after = int(
                            response.headers.get(
                                "Retry-After", self.initial_retry_delay * (2**attempt)
                            )
                        )
                        retry_time = self._get_retry_time(attempt, retry_after)
                        logger.warning(
                            f"Too many requests, retrying after {retry_time:.2f} seconds..."
                        )
    ...
    async def _convert_mp3_to_wav_async(mp3_file: str, wav_file: str) -> None:
        ...
        try:
            await loop.run_in_executor(
                None, AudioSegment.from_mp3(mp3_file).export, wav_file, "wav"
            )
        finally:
    ...
    def synthesize(
        ...
        raise ElevenLabsAPIError(
            f"Failed to synthesize speech after {self.max_retries} attempts"
        )
