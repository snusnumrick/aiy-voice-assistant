...
class StressTool:
    ...
    def tool_definition(self) -> Tool:
        return Tool(
            ...
            iterative=True,
            parameters=[
                ToolParameter(name='word', type='string', description='A Russian word to add stress marks to')
            ],
            required=["word"],
    ...
    def do_stress(self, parameters: Dict[str, any]) -> str:
        if 'word' in parameters:
            ...
            import requests
            response = requests.post(self.morpher_url, headers=self.headers, data=parameters['word'].encode('utf-8'))
    ...
    async def do_stress_async(self, parameters: Dict[str, any]) -> str:
        if 'word' in parameters:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.morpher_url, headers=self.headers,
                                        data=parameters['word'].encode('utf-8')) as response:
                    self._stop_processing()
