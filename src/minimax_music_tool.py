...
try:
    ...
    from mutagen.id3 import ID3NoHeaderError, ID3, APIC, TIT2, TPE1, TALB, TCON, COMM
    HAS_MUTAGEN = True
except ImportError:
    HAS_MUTAGEN = False
                ...
                async with session.post(
                    ...
                    if HAS_MUTAGEN:
                        try:
                            ...
                            audio.tags.add(TPE1(encoding=3, text=["MiniMax AI"]))  # Artist
                            audio.tags.add(TALB(encoding=3, text=["Generated Music"]))  # Album
                            audio.tags.add(TCON(encoding=3, text=["Electronic"]))  # Genre
                            audio.tags.add(COMM(encoding=3, text=[
                                f"Generated by MiniMax AI\n"
                                f"Prompt: {prompt}\n"
                                f"Lyrics: {lyrics[:100]}{'...' if len(lyrics) > 100 else ''}\n"
                                f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(timestamp))}"
                            ]))  # Comments with lyrics and prompt
                        ...
                        logger.debug("Skipping metadata tags (mutagen not available)")
